    ...
    <SafeAreaView style={S.screen}>
      ...
      >
        <View style={isDesktop ? S.contentDesktop : undefined}>
          ...
          {/* Pending gifts — same row style as other admin lists */}
          <View style={S.pendingBlock} accessibilityRole="summary">
            <View style={S.sectionRow}>
              <Text style={S.sectionTitle}>
                מתנות למימוש
                {!pendingLoading && pendingRedemptions.length > 0
            ...
            {pendingLoading ? (
              ...
              <View style={AS.row}>
                <View style={[AS.rowAvatar, AS.rowAvatarSecondary]}>
                  <Trophy size={20} color={Colors.muted} />
                </View>
                <View style={AS.rowLeft}>
                  <Text style={AS.rowTitle}>אין מתנות למימוש</Text>
                  <Text style={AS.rowSub}>כשכיתה תירשם לפרס — תופיע כאן</Text>
                </View>
              </View>
            ) : (
              pendingRedemptions.map((r) => {
                const scheme = getClassColorScheme(r.classes?.grade || r.classes?.name || '');
                return (
                  <TouchableOpacity
                    key={r.id}
                    onPress={() => router.push('/admin/redemptions' as any)}
                    style={[AS.row, pointer]}
                    accessibilityRole="button"
                    accessibilityLabel={`כיתה ${r.classes?.name ?? ''}, מתנה ${r.gifts?.name ?? 'ללא שם'}, ממתין למימוש`}
                  >
                    <View style={[AS.rowAvatar, { backgroundColor: scheme.bg }]}>
                      <Text
                        style={[AS.rowAvatarText, { color: scheme.text, fontSize: 13 }]}
                        numberOfLines={1}
                      >
                        {r.classes?.name ?? '—'}
                      </Text>
                    ...
                    <View style={AS.rowLeft}>
                      <Text style={AS.rowTitle} numberOfLines={1}>
                        {r.gifts?.name ?? 'מתנה'}
                      </Text>
                      <Text style={AS.rowSub} numberOfLines={1}>
                        {r.note
                          ? r.note
                          : `נרשם ${moment(r.redeemed_at).format('DD/MM/YY')}`}
                      </Text>
                    </View>
                    <View style={S.pendingChip}>
                      <Text style={S.pendingChipText}>ממתין</Text>
                    </View>
                  </TouchableOpacity>
                );
              })
...
const S = StyleSheet.create({
  ...
  // ── Pending redemptions (admin list rows) ──
  pendingBlock: { marginBottom: 20 },
  sectionRow: {
