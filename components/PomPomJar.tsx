...
const JAR_W     = 190;
const JAR_H     = 250;
const POM_D     = 40;     // base diameter
const COLS      = 4;
const SPACING_X = Math.floor((JAR_W - 12) / COLS); // ~44 px
const SPACING_Y = Math.round(POM_D * 0.60);         // ~24 px — rows overlap a bit

// ── Pre-calculated rest positions (bottom → top, zigzag rows) ─────────────────
...
const LAYOUT = Array.from({ length: MAX_POMS }, (_, i) => {
  ...
  return {
    ...
    size:     POM_D - 4 + (i % 3) * 4,   // 36 / 40 / 44 px variation
    rotation: (i * 47) % 360,             // pseudo-random rotation
  };
});

// ── Single pom-pom — uses the real image ──────────────────────────────────────
...
export function PomPomJar({ current, goal }: PomPomJarProps) {
  ...
  // One Animated.Value per slot; starts above the jar and springs to 0
  ...
  useEffect(() => {
    ...
    if (to > from) {
      ...
      // Snap removed poms back above the jar instantly
